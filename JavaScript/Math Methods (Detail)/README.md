...
`Math.hypot(...values)` 是 JavaScript 中用來計算多個數值平方和的平方根的內建方法，用來計算多維空間中一個點到原點的距離，或是用來求解直角三角形的斜邊長度。在二維空間，可以用來計算兩個座標 (x, y) 到原點的距離；在三維或更高維空間，也同樣適用。
...
`Math.hypot(...values)` 是一個用來計算多個數值平方和的平方根的函數，特別適合用來計算二維、三維甚至更高維空間中一個點到原點的距離。`Math.hypot(...values)` 不僅提供簡潔的語法，還能有效避免手動計算時可能出現的數值溢出問題，常用於距離計算、物理模擬、遊戲開發以及工程學中的應用。
...
- x：要計算的數字。這個數字會被轉換成一個 32 位元整數，如果 x 是一個非整數的值，會先進行向下取整 (即取整至最接近的整數)。
...
- 輸入與輸出
    ...
    - 如果 x 為 0，返回值為 32，因為 0 的二進制表示為全零。
...
`Math.clz32(x)` 是用於計算一個數字在 32 位元整數表示中前導零數量的函數，在位元運算和數據處理中非常有用，能夠幫助開發人員快速獲得數字的位元特性，並在某些情況下提高性能。通過提供簡單的語法和穩定的計算方式，`Math.clz32(x)` 成為處理整數時的一個重要工具。

<br />

## Math.imul(a, b)

`Math.imul(a, b)` 是 JavaScript 中用來進行兩個 32 位元整數相乘的內建方法，專門用來執行高效能的整數乘法，並返回正確的 32 位元有號整數結果。`Math.imul(a, b)` 能夠避免 JavaScript 常見的數字溢出或精度問題，特別是在處理大數時特別有用。
...
- 作用機制
    ...
    - `Math.imul(a, b)` 會先將 a 和 b 轉換為 32 位元整數 (範圍為 -2^31 到 2^31-1)，然後進行相乘運算，最後返回一個 32 位元有號整數結果。
...
- 範例
    ...
    - 處理大數值
        ...
        在這個範例中，0x7FFFFFFF 是 JavaScript 中的最大 32 位元有號整數 (2^31-1)，與 2 相乘的結果會溢出，並返回 -2。這是因為 `Math.imul(a, b)` 處理的是 32 位元有號整數，超過範圍時會自動進行溢出處理。        
...
- 浮點數取整數：`Math.imul(a, b)` 會將輸入的浮點數轉換為整數來進行運算，因此如果輸入的是浮點數，結果可能與預期的不同。

- 溢出處理：`Math.imul(a, b)` 處理的是 32 位元有號整數，因此如果乘法結果超過這個範圍，會進行溢出處理並返回相應的結果。

### 總結

`Math.imul(a, b)` 是用來進行 32 位元整數乘法的高效方法，適合在需要進行位元運算或高效整數計算的情況使用。這個方法不僅能避免 JavaScript 的浮點數精度問題，還能有效處理 32 位元整數的乘法，確保計算結果符合預期。在處理大數值和需高效能的情況下也提供了很好的解決方案。
...
### 注意事項
    ...
    - 溢出與下溢：如果輸入的數字超出 32 位元單精度浮點數的範圍，結果會是 Infinity 或 -Infinity。若數字非常小，則可能會被轉換為 0。

### 總結

`Math.fround(x)` 用於將數字轉換為最接近的 32 位元單精度浮點數。在許多需要精度和記憶體控制的情況下，`Math.fround(x)` 提供了一個有效的解決方案。雖然會有精度損失，但在圖形處理、科學計算和性能要求較高的情況下，都能幫助提升運算效能並減少記憶體使用。

<br />

## Math.expm1(x)

`Math.expm1(x)` 是 JavaScript 中的一個內建方法，用來計算數字 x 的 exponential 函數減去 1，即 e^x - 1 的值，e 是自然對數的底數，約等於 2.718。這個方法的主要用途是當 x 很接近 0 時，能夠更精確計算 e^x - 1，因為直接計算 `Math.exp(x) - 1` 可能會因為浮點數精度的限制而導致結果不夠精確。
...
- 數學背景
    ...
    - `Math.expm1(x)` 其實是在計算「指數函數 (exponential function)」的變化量，即 e^x - 1。在數學中，當 x 越來越接近 0 時，e^x - 1 的結果也會越來越接近 0。這時如果直接使用 `Math.exp(x) - 1`，由於 JavaScript 浮點數的限制，可能會產生精度誤差。而 `Math.expm1(x)` 就是為了避免這種精度問題而設計的。

- 使用原因

    - 當 x 接近 0 時，計算 e^x - 1 非常常見於數學和工程計算中。例如：在一些金融計算、物理模擬和機率運算裡，指數函數的變化是核心運算之一。
...
- 範例
    ...
    在這個範例中，計算的是 e^1 - 1，結果接近於 2.718 - 1，也就是 1.718。
    ...
    當輸入負數時，`Math.expm1(-1)` 計算的是 e^-1 - 1，即 1/e - 1，這是一個負數。

- 應用場景

    - 科學與工程計算：在處理一些增長模型、複利計算，或物理上的熱力學公式時，經常需要計算 e^x - 1 的值。`Math.expm1(x)` 使得這些計算更加精確，特別是在處理非常小的數字時。
...
- 數學推導與比較
    ...
    - `Math.expm1(x)`：專門用來避免這個問題，尤其適用於小的 x，能確保計算 e^x - 1 更加精確。
...
`Math.expm1(x)` 是用來精確計算 e^x - 1 的方法，特別是在處理接近 0 的數字時，比直接計算 `Math.exp(x) - 1` 更精確。`Math.expm1(x)` 特別是用在科學計算、金融分析、以及其他高精度情況下。透過 `Math.expm1(x)`，可以避免由於浮點數精度限制而導致的誤差，使得計算結果更為準確。
...
- x：必須是一個實數 (正數、負數或 0)，但必須滿足 x > -1，因為如果 x ≤ -1，則 ln(1 + x) 沒有定義或會導致數學錯誤。
...
- 數學背景
    ...
    - 當 x 越接近 0，ln(1 + x) 的結果越接近 0。如果直接使用 `Math.log(1 + x)`，在 x 很小的時候，因為浮點數的限制，可能會產生誤差，而 `Math.log1p(x)` 就是為了解決這個問題，提供更精確的結果。
...
- x 值範圍：`Math.log1p(x)` 的有效範圍是 x > -1，因為當 x ≤ -1 時，1 + x 會變為 0 或負數，而自然對數在這些情況下沒有定義。當輸入 x ≤ -1 時，結果會是 NaN (非數值)。
