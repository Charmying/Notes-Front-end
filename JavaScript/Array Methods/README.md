...
- `copyWithin()`：在陣列內部複製元素到其他位置 (會修改原陣列)。

### 返回新陣列的方法

- `slice()`：返回一個新的陣列，包含從開始索引到結束索引 (不包含結束索引) 的部分元素。

- `concat()`：合併數個陣列，返回一個新陣列。

- `flat()`：將多維陣列展平成一維陣列。

- `flatMap()`：先對陣列進行 map 操作，再將結果展平成一維陣列。

- `toSorted()`：返回一個已排序的新陣列，不改變原本的陣列。

- `toReversed()`：返回一個倒序的新陣列，不改變原本的陣列。

- `toSpliced()`：返回一個已修改的新陣列，用法類似 `splice()`，但不改變原本的陣列。

- `with()`：返回一個指定索引值被替換的新陣列，不改變原本的陣列。

### 轉換和迭代方法

- `toString()`：將陣列轉換為字串，元素之間以逗號分隔。

- `join()`：將陣列轉換為字串，可以指定分隔符號。

- `forEach()`：為陣列中的每個元素執行一次提供的函式。

- `map()`：創建一個新陣列，其元素是調用函式處理每個元素後的結果。

- `filter()`：創建一個新陣列，其結果是滿足條件的所有元素。

- `reduce()`：將陣列中的每個元素依次累積，最終為單一值。

- `reduceRight()`：從右到左對陣列中的元素進行累積，最終為單一值。

### 搜尋和判斷方法

- `includes()`：判斷陣列是否包含某個值，返回布林值。

- `indexOf()`：返回指定元素在陣列中的第一個索引，若未找到則返回 -1。

- `lastIndexOf()`：返回指定元素在陣列中的最後一個索引，若未找到則返回 -1。

- `find()`：返回符合條件的第一個元素，否則返回 undefined。

- `findIndex()`：返回符合條件的第一個元素的索引，否則返回 -1。

- `some()`：判斷陣列中是否至少有一個元素滿足條件，返回布林值。

- `every()`：判斷陣列中的所有元素是否都滿足條件，返回布林值。

### 迭代器和靜態方法

- `entries()`：返回一個新的陣列迭代器物件，包含鍵值對 (key-value pair)。

- `keys()`：返回一個新的陣列迭代器物件，包含每個索引。

- `values()`：回傳一個陣列值 (values) 的迭代器，可用 `for...of` 取出每個值。

- `from()`：將類陣列物件或可迭代物件轉換為陣列。

- `isArray()`：檢查給定的值是否為陣列，返回布林值。

### 其他方法

- `valueOf()`：返回陣列的原始值 (通常與陣列本身相同)。

- `at()`：取得指定索引的值 (支援負索引，從陣列尾端開始算)。

- `group()`：根據 callback function 的回傳值分組陣列元素，回傳一個物件 (Object)。

- `groupToMap()`：和 `group()` 類似，但回傳 Map 物件，更適合需要鍵值為非字串的情境。

- `delete` 運算符：刪除陣列中的某個元素，但不會改變陣列的長度，會留下 empty。
